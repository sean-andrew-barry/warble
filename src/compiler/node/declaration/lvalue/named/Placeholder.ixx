export module node.declaration.lvalue.named.placeholder;
...
namespace node::declaration::lvalue::named {
  ...
  export class Placeholder : public Named {
  private:
    std::vector<Symbol> types; // Parameters are templates that can have many types
  public:
    using Named::Named;

    Placeholder(const lexical::source::Span& span, node::Scope& scope, std::string_view name) noexcept;
    Placeholder(const lexical::source::Span& span, node::Scope& scope, const std::string& name) noexcept;
