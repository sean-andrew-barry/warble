...
namespace ir {
  ...
  public:
    ...
    /* Walk parents until we hit a function or module.
    *  • returns 'true' if function  →  base = RSP
    *  • returns 'false' if module   →  base = RIP
    *  • dispOut = sum of Displacement() on the path              */
    static inline bool resolve_base(const ir::Symbol& leaf, int32_t& dispOut) {
      dispOut = 0;
      const ir::Symbol* p = &leaf;
      while (!p->IsFunction() && !p->IsModule()) {
        dispOut += p->Displacement();
        p = &p->Parent();
      }
    ...
    static inline uint8_t lo3(unsigned v) { return v & 7; }

    static inline uint8_t mod_field(int32_t disp, uint8_t baseLo3) {
      /* mod == 00  but  [rbp] / [r13] must use a disp8 = 0 */
      if (disp == 0 && baseLo3 != 5)                 return 0;   // no disp
      if (static_cast<int8_t>(disp) == disp)         return 1;   // disp8
      return 2;                                                  // disp32
    }

    /* Emit signed disp8 / disp32 chosen by 'mod'. */
    static inline void emit_disp(uint8_t mod, int32_t disp) {
      if (mod == 1)
          Code().Emit8(static_cast<uint8_t>(disp));
      else if (mod == 2 ||                      // disp32
              (mod == 0 && disp != 0))        // RIP-rel always disp32
          Code().Emit32(static_cast<uint32_t>(disp));
    }

    static inline uint8_t Mod(bool local, int32_t disp) {
      /* mod == 00  but  [rbp] / [r13] must use a disp8 = 0 */
      if (disp == 0 && local) return 0;   // no disp
      if (static_cast<int8_t>(disp) == disp) return 1; // disp8
      return 2; // disp32

      Code().Emit8((mod << 6) | (ext << 3) | (local ? 4 : 5));
    }

    inline void Mod(uint8_t ext, int32_t disp, bool local) {
      if (disp == 0 && local) {
        Code().Emit8((ext << 3) | (local ? 4 : 5)); // No displacement
      } else if (static_cast<int8_t>(disp) == disp) {
        Code().Emit8((1 << 6) | (ext << 3) | (local ? 4 : 5)); // disp8
      } else {
        Code().Emit8((2 << 6) | (ext << 3) | (local ? 4 : 5)); // disp32
      }
    }

    inline void Displacement(int32_t displacement, bool local) {
      if (displacement == 0 && local) {
        return; // No displacement
      } else if (static_cast<int8_t>(displacement) == displacement) {
        Code().Emit8(static_cast<uint8_t>(displacement));
      } else {
        Code().Emit32(static_cast<uint32_t>(displacement));
      }
    }

    inline void ModRM(uint8_t ext, const ir::Symbol& rm) {
      if (rm.IsAllocated()) {
        // Register form
        Code().Emit8(0xC0 | (ext << 3) | (rm.Register() & 7));
        return;
      }

      // Memory form - determine implicit base & displacement
      int32_t disp = rm.Displacement();
      bool local = rm.IsLocal(disp); // Adds parent displacements
      bool small = static_cast<int8_t>(disp) == disp;

      uint8_t baseLo3 = local ? 4 : 5; // RSP or sentinel for RIP

      Mod(ext, disp, local);
      Code().Emit8((mod << 6) | (ext << 3) | (local ? 4 : 5));

      // [rsp] or any address that needs SIB
      if (local) Code().Emit8((4 << 3) | 4); // scale=1,index=none,base=rsp

      Displacement(disp, local);
    }

    inline void ModRM(const ir::Symbol& reg, const ir::Symbol& rm) {
      if (rm.IsAllocated()) {
        /* register–register :  11 | reg | r/m */
        Code().Emit8(0xC0 | ((reg.Register() & 7) << 3) | (rm.Register() & 7));
        return;
      }

      /* register  ←/→  memory */
      int32_t disp = 0;
      bool local = rm.IsLocal(disp);
      uint8_t baseLo3 = local ? 4 : 5; // 4 = rsp/r12, 5 = RIP sentinel
      uint8_t mod = mod_field(disp, baseLo3);

      Code().Emit8((mod << 6) | ((reg.Register() & 7) << 3) | baseLo3);

      // need SIB
      if (baseLo3 == 4) Code().Emit8((4 << 3) | 4); // scale=1,index=none,base=rsp

      emit_disp(mod, disp);
    }

    inline void ModRM(const ir::Symbol& reg, const ir::Symbol& base, const ir::Symbol& index, int32_t disp, uint8_t scale) {
      /* Always memory: rm = 100 to summon SIB */
      uint8_t mod = mod_field(disp, 4);

      Code().Emit8((mod << 6) | (lo3(reg.Register()) << 3) | 4); // rm = 100 = uses SIB

      /* SIB byte */
      uint8_t sib = (scale << 6) | (lo3(index.Register()) << 3) | lo3(base.Register());
      Code().Emit8(sib);
