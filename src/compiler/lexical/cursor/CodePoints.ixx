...
namespace lexical::cursor {
  ...
  constexpr char32_t END() { return U'\0'; }

  export class CodePoints : public lexical::Cursor<std::vector<char32_t>, END> {
  private:
  public:
    using Super::Super;
    using Super::Peek;
    ...
    constexpr bool Fits(const std::string_view text) const { return Size() >= text.size(); }
    constexpr bool Fits(const std::u32string_view text) const { return Size() >= text.size(); }

    constexpr bool Check(const char32_t c) const {
      return Super::Peek() == c;
    }
    ...
    constexpr bool Check(const std::string_view text) const {
      ...
      for (size_t i = 0; i < text.size(); ++i, ++temp) {
        if (static_cast<char32_t>(text[i]) != *temp) {
          return false;
        }
      }
    ...
    constexpr bool Check(const std::u32string_view text) const {
      if (!Fits(text)) return false; // If it doesn't fit, it can't match
    ...
    constexpr bool Match(const char c) {
      auto iter = cbegin();
      if (iter != cend() && *iter == static_cast<char32_t>(c)) {
        Advance();
        return true;
      } else {
        return false;
    ...
    constexpr bool Match(const char32_t c) {
      auto iter = cbegin();
      if (iter != cend() && *iter == c) {
        Advance();
        return true;
    ...
    constexpr bool Match(const std::u32string_view text) {
      if (!Check(text)) return false; // Compare the text to the content
    ...
    // constexpr std::span<char32_t> Capture(const std::u32string_view text) {
    //   if (!Check(text)) return std::span<char32_t>{cbegin(), 0};

    //   std::span<char32_t> captured{cbegin(), text.size()};
    ...
    constexpr std::u32string_view Capture(const std::u32string_view text) {
      if (!Check(text)) return std::u32string_view{cbegin(), cbegin()};

      std::u32string_view captured{cbegin(), cbegin() + text.size()};
    ...
    constexpr bool Keyword(const std::u32string_view text) {
      if (!Check(text)) return false; // Compare the text to the content
      if (IsAlNum(*(cbegin() + text.size()))) return false; // The next character cannot be an alphanumeric character
    ...
    constexpr static bool IsBreak(const char32_t c) { return c == U'\n' || c == U'\u2028' || c == U'\u2029'; }
    constexpr static bool IsSpace(const char32_t c) { return WHITESPACE[static_cast<uint8_t>(c)]; }
    constexpr static bool IsAlpha(const char32_t c) { return ALPHA[static_cast<uint8_t>(c)]; }
    constexpr static bool IsAlNum(const char32_t c) { return ALNUM[static_cast<uint8_t>(c)]; }
    constexpr static bool IsUpper(const char32_t c) { return UPPER[static_cast<uint8_t>(c)]; }
    constexpr static bool IsLower(const char32_t c) { return LOWER[static_cast<uint8_t>(c)]; }
    constexpr static bool IsDigit(const char32_t c) { return DIGIT[static_cast<uint8_t>(c)]; }
    constexpr static bool IsPunct(const char32_t c) { return PUNCT[static_cast<uint8_t>(c)]; }
    constexpr static bool IsSlash(const char32_t c) { return SLASH[static_cast<uint8_t>(c)]; }
    constexpr static bool IsOctal(const char32_t c) { return OCTAL[static_cast<uint8_t>(c)]; }
    constexpr static bool IsHex  (const char32_t c) { return HEX  [static_cast<uint8_t>(c)]; }
    constexpr static bool IsIdent(const char32_t c) { return IDENTIFIER[static_cast<uint8_t>(c)]; }
    constexpr static bool IsBinary(const char c) { return c == U'0'|| c == U'1'; }
    constexpr static bool IsIdentStart(const char32_t c) { return IDENTIFIER_START[static_cast<uint8_t>(c)]; }
    constexpr static bool IsValueStart(const char32_t c) { return VALUE_START[static_cast<uint8_t>(c)]; }
    constexpr static bool IsStatementStart(const char32_t c) { return STATEMENT_START[static_cast<uint8_t>(c)]; }
    constexpr static bool IsUnaryPrefixStart(const char32_t c) { return UNARY_PREFIX_START[static_cast<uint8_t>(c)]; }
    constexpr static bool IsUnaryPostfixStart(const char32_t c) { return UNARY_POSTFIX_START[static_cast<uint8_t>(c)]; }
    constexpr static bool IsCallablePrefixStart(const char32_t c) { return CALLABLE_PREFIX_START[static_cast<uint8_t>(c)]; }
    constexpr static bool IsCallablePostfixStart(const char32_t c) { return CALLABLE_POSTFIX_START[static_cast<uint8_t>(c)]; }
    constexpr static bool IsBinaryStart(const char32_t c) { return BINARY_START[static_cast<uint8_t>(c)]; }
    constexpr static bool IsModifierStart(const char32_t c) { return MODIFIER_START[static_cast<uint8_t>(c)]; }
    constexpr static bool IsNumberStart(const char32_t c) { return NUMBER_START[static_cast<uint8_t>(c)]; }
    constexpr static bool IsPossibleSpaceStart(const char32_t c) { return POSSIBLE_SPACE_START[static_cast<uint8_t>(c)]; }
    constexpr static bool IsPossibleIdentifierStart(const char32_t c) { return POSSIBLE_IDENTIFIER_START[static_cast<uint8_t>(c)]; }
    constexpr static bool IsURLProtocol(const char32_t c) { return URL_PROTOCOL[static_cast<uint8_t>(c)]; }
    constexpr static bool IsURLHost(const char32_t c) { return URL_HOST[static_cast<uint8_t>(c)]; }
    constexpr static bool IsURLPathname(const char32_t c) { return URL_PATHNAME[static_cast<uint8_t>(c)]; }
    constexpr static bool IsURLSearch(const char32_t c) { return URL_SEARCH[static_cast<uint8_t>(c)]; }
    constexpr static bool IsURLHash(const char32_t c) { return URL_HASH[static_cast<uint8_t>(c)]; }
    constexpr static bool IsURLAuthority(const char32_t c) { return URL_AUTHORITY[static_cast<uint8_t>(c)]; }
    constexpr static bool IsFilePath(const char32_t c) { return FILE_PATH[static_cast<uint8_t>(c)]; }
  };
};