export module code.arm64.encoder;

import code.encoder;
import code.arm64.registers;
...
namespace code::arm64 {
  ...
  private:
    uint32_t IMM12(uint32_t imm12) { return (imm12 & 0xFFF) << 10; }
    uint32_t SH(bool sh) { return ((sh ? 1 : 0) & 0x1) << 22; }

    uint32_t RM(W rm) { return (rm.Value() & 0x1F) << 16; }
    uint32_t RN(W rn) { return (rn.Value() & 0x1F) << 5; }
    uint32_t RD(W rd) { return (rd.Value() & 0x1F); }
    uint32_t RA(W ra) { return (ra.Value() & 0x1F) << 10; }

    uint32_t RM(X rm) { return (rm.Value() & 0x1F) << 16; }
    uint32_t RN(X rn) { return (rn.Value() & 0x1F) << 5; }
    uint32_t RD(X rd) { return (rd.Value() & 0x1F); }
    uint32_t RA(X ra) { return (ra.Value() & 0x1F) << 10; }
  public:
    void ADD_R32_R32(W rn, W rm, W rd) { Emit32(0x0B000000 | RM(rm) | RN(rn) | RD(rd)); }
    void ADD_R64_R64(X rn, X rm, X rd) { Emit32(0x8B000000 | RM(rm) | RN(rn) | RD(rd)); }

    void ADD_I12_R32(uint32_t i, W rn, W rd, bool sh = false) { Emit32(0x11000000 | SH(sh) | IMM12(i) | RN(rn) | RD(rd)); }
    void ADD_I12_R64(uint32_t i, X rn, X rd, bool sh = false) { Emit32(0x91000000 | SH(sh) | IMM12(i) | RN(rn) | RD(rd)); }

    void MADD_R32_R32(W rn, W rm, W ra, W rd) { Emit32(0x1B000000 | RM(rm) | RA(ra) | RN(rn) | RD(rd)); }
    void MADD_R64_R64(X rn, X rm, X ra, X rd) { Emit32(0x9B000000 | RM(rm) | RA(ra) | RN(rn) | RD(rd)); }
    ...
    // // 0xD4400000, /* HLT hlt */
    // // 0xD4A00001, /* DCPS1 dcps1 */
    // // 0xD4A00002, /* DCPS2 dcps2 */
    // // 0xD4A00003, /* DCPS3 dcps3 */
    // /* System */
    // // 0xD500401F, /* MSR msrimm */
    // // 0xD503201F, /* HINT hint */
    ...
    // // 0xD5080000, /* SYS sys */
    // // 0xD5100000, /* MSR msr */
    // // 0xD5280000, /* SYSL sysl */
    // // 0xD5300000, /* MRS mrs */
    // /* Unconditional branch (register) */
    // 0xD61F0000, /* BR br */
    // 0xD63F0000, /* BLR blr */
    // 0xD65F0000, /* RET ret */
    // // 0xD69F03E0, /* ERET eret */
    // // 0xD6BF03E0, /* DRPS drps */
